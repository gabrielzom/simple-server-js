...
import { executeScript } from './database.js'
...
app.listen(proccess.env.PORT || 8080, () => console.log(`App listen in port: http://localhost:8080`))
...
const executeRequest = (req, res) => {
  ...
  if (['/start-trip', '/end-trip', '/make-payment']
    ...
    Promise.all(scriptsList.map(script => executeScript(script)))
      .then(result => res.status(200).json({rows: result[0][0], columns: result[0][1]}))
